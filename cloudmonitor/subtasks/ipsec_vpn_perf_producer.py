...
from cloudmonitor.subtasks.ipsec_vpn_perf_collector import IpsecVpnPerfCollector
...
class IpsecVpnPerfProducer(SubTaskBase):

    def run(self, context):
        with context.session.begin(subtransactions=True):
            db_ftp = context.session.query(models.Ftp) \
                .join(models.SubTask) \
                .join(models.Task, models.Task.name == IpsecVpnPerfCollector.__name__) \
                .filter(or_(models.Ftp.status == models.FtpStatus.DOWNLOAD_SUCCESS.value,
            ...
            try:
                context.rocketmq_producer.send_sync(context.rocketmq_producer.performance_topic, 'test-key',
                                                    'test-tag',
                                                    json.dumps(body))
